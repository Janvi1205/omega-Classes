...
const BentoCardGrid: React.FC<{
  ...
  <div
    className={`bento-section ${className} select-none relative`}
    ref={gridRef}
  >
    {children}
  </div>
      ...
      <style>
        {`
          ...
          .card--border-glow:hover {
            box-shadow: 0 4px 20px rgba(59, 130, 246, 0.2), 0 0 20px rgba(${glowColor}, 0.1);
          }
      ...
      <BentoCardGrid gridRef={gridRef}>
        <div className="grid gap-4 sm:gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {cards.map((card, index) => {
            const baseClassName = `card flex flex-col justify-between relative aspect-[4/3] min-h-[180px] w-full max-w-full p-4 sm:p-6 rounded-xl border border-border bg-card hover:bg-accent/5 transition-all duration-300 ease-in-out hover:-translate-y-1 hover:shadow-lg ${
              ...
              return (
                ...
                >
                  <div className="card__header flex justify-between gap-3 relative">
                    <span className="card__label text-sm text-muted-foreground">{card.label}</span>
                    {card.icon && (
                  ...
                  <div className="card__content flex flex-col relative">
                    <h3
                      className={`card__title font-semibold text-base text-foreground m-0 mb-2 ${textAutoHide ? "text-clamp-1" : ""}`}
                    >
                      {card.title}
                    </h3>
                    <p
                      className={`card__description text-sm leading-5 text-muted-foreground ${textAutoHide ? "text-clamp-2" : ""}`}
                    >
                      {card.description}
                    </p>
                  </div>
